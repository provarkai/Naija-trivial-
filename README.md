...
**Fact-check status:** most of the bank was written from general knowledge, unverified — but the highest-risk ~65 questions (every "hard"-difficulty one, plus specific-fact "medium" ones: award years, club affiliations, match results, attributions) have now been checked one by one against live sources (Wikipedia, CAF, FIFA, Grammy.com, Billboard, major Nigerian outlets). That pass caught and fixed **five** real errors so far:
...
The remaining ~335 questions — mostly safer, more generic facts (capital cities, currency names, basic geography) — haven't been individually re-verified; still worth a full pass before shipping, but the error rate found in the highest-risk sample (5 of ~65, all now fixed) is the best available signal on what to expect from the rest.
